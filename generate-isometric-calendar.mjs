 ...
 * Extended to support full-year (12 months) view
 */
...
const DURATION = process.env.DURATION || 'full-year';
 ...
 * Now supporting full-year (12 months) with larger viewBox
 */
function generateSVG(weeks, stats, duration) {
  const size = 6;
  
  // Determine viewBox height based on duration
  const viewBoxHeight = duration === 'full-year' ? 350 : 170;
